--CREATE VIEW tournament_leaderboard.leaderboard_tournament_position AS (
WITH cte_leaderboard_round AS
  (
   SELECT
       ...
       ,TournamentID
       ,PlayerTournamentID
   FROM `tournament_leaderboard.leaderboard_round_stats`
  )
,
cte_leaderboard_hole AS
  (
   SELECT
      ...
      ,ToParOnRound
      ,StrokesOnRound
   FROM `tournament_leaderboard.leaderboard_hole_stats`
  )
,
